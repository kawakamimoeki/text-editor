...
export const dropCursor = (kaku: Kaku, position: { x: number; y: number }) => {
  let x = 0;
  let y = 0;
  let index = 0;
    ...
    if (y > position.y) {
      for (let j = 0; j < kaku.state.visualLines[i].length; j++) {
        ...
        if (x + width > position.x) {
          kaku.state.selections = [{ ...selection, from: index, to: index }];
          return;
        }
      ...
      if (i === kaku.state.visualLines.length) {
        kaku.state.selections = [
          { ...selection, from: index + 1, to: index + 1 },
        ];
        return;
      }
    }
  ...
  kaku.state.selections = [
    {
      ...selection,
      from: kaku.state.content.length,
      to: kaku.state.content.length,
    },
  ];
  return;
};
